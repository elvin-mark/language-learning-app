...
class AIEngine:
    ...
    def generate_response(
        ...
        if scenario:
            system_prompt = ai_prompts.get_roleplay_prompt(
                scenario, target_language, prompt_provider
            )
        else:
            system_prompt = ai_prompts.get_system_prompt(
                target_language, prompt_provider
            )
        ...
        try:
            ...
            if llm_type == "local":
                ...
                try:
                    ...
                    usage["model_name"] = raw_msg.response_metadata.get(
                        "model_name", "local-model"
                    )
    ...
    def explain_snippet(
        ...
        structured_llm = self.get_llm(
            llm_type, cloud_provider, local_llm_url, output_model=AISystemResponse
        )
        prompt_provider = "local" if llm_type == "local" else cloud_provider
        system_prompt = ai_prompts.get_explanation_prompt(
            text, target_language, prompt_provider
        )

        messages = [
            ("system", system_prompt),
            ("human", f"Please explain this {target_language} snippet: {text}"),
        ]
        prompt = ChatPromptTemplate.from_messages(messages)
        chain = prompt | structured_llm
    ...
    def get_suggestion(
        ...
        llm_simple = self.get_llm(
            llm_type, cloud_provider, local_llm_url, output_model=SuggestionResponse
        )
        prompt_provider = "local" if llm_type == "local" else cloud_provider
        system_prompt = ai_prompts.get_suggestion_prompt(
            target_language, prompt_provider
        )
        ...
        messages.append(
            ("human", "Help! I don't know what to say next. Suggest something for me.")
        )
        ...
        try:
            ...
            if raw and hasattr(raw, "content"):
                ...
                parsed_dict = json.loads(clean_json)
                return {
                    "suggestion": parsed_dict.get("suggestion"),
                    "translation": parsed_dict.get("translation"),
                }
    ...
    async def generate_scenario(
        ...
        structured_llm = self.get_llm(
            llm_type, cloud_provider, local_llm_url, output_model=ScenarioInfo
        )
        prompt_provider = "local" if llm_type == "local" else cloud_provider
        prompt = ai_prompts.get_scenario_generation_prompt(
            topic, target_language, prompt_provider
        )
        ...
        try:
            ...
            return {
                ...
                "initial_message": "안녕하세요! 만나서 반가워요."
                if target_language == "Korean"
                else "こんにちは！よろしくお願いします。",
    ...
    async def get_writing_assistant(
        ...
        structured_llm = self.get_llm(
            ...
            cloud_provider,
            local_llm_url,
            output_model=WritingAssistantResponse,
        )
        prompt_text = ai_prompts.get_writing_assistant_prompt(
            text, target_language, scenario
        )
                ...
                "variations": [
                    {
                        "label": "Natural",
                        "text": text,
                        "explanation": "Failed to generate variations. Using original.",
                    }
                ]
            }
    ...
    async def generate_reading_task(
        ...
        structured_llm = self.get_llm(
            llm_type, cloud_provider, local_llm_url, output_model=ReadingPassageInfo
        )
        prompt_provider = "local" if llm_type == "local" else cloud_provider
        prompt_text = ai_prompts.get_reading_generation_prompt(
            topic, target_language, difficulty, prompt_provider
        )
    ...
    async def generate_journal_prompt(
        ...
        from backend.ai_models import JournalPromptResponse

        structured_llm = self.get_llm(
            llm_type, cloud_provider, local_llm_url, output_model=JournalPromptResponse
        )
        prompt_text = ai_prompts.get_journal_prompt_generation_prompt(
            target_language, topic
        )
        ...
        try:
            ...
            raise Exception("AI failed to return valid journal prompt")
        except Exception as e:
            print(f"Error generating journal prompt: {str(e)}")
            return {
                "prompt": f"Write about your favorite place to visit in a country where they speak {target_language}.",
                "explanation": "Focuses on descriptive vocabulary and personal experience.",
            }
    ...
    async def review_journal_entry(
        ...
        """Analyzes a journal entry and provides structured feedback."""
        from backend.ai_models import JournalReviewResponse

        structured_llm = self.get_llm(
            llm_type, cloud_provider, local_llm_url, output_model=JournalReviewResponse
        )
        prompt_provider = "local" if llm_type == "local" else cloud_provider
        prompt_text = ai_prompts.get_journal_review_prompt(
            content, target_language, prompt_provider
        )
        ...
        try:
            ...
            raise Exception("AI failed to return valid journal review")
        except Exception as e:
            print(f"Error reviewing journal entry: {str(e)}")
            raise e
