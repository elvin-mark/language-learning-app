...
async def generate_exercise(exercise_request: schemas.ExerciseRequest, db: Session = Depends(get_db)):
...
async def submit_exercise(submission: schemas.Submission, db: Session = Depends(get_db)):
...
async def get_review_history(db: Session = Depends(get_db)):
    ...
    return [
        schemas.ExerciseListItem(exercise_id=101, grade=85, type="Writing", date=datetime.utcnow()),
        schemas.ExerciseListItem(exercise_id=102, grade=90, type="Flashcard", date=datetime.utcnow())
    ]
...
async def get_grammar_mastery(db: Session = Depends(get_db)):
    # This will be implemented later with actual logic to fetch from DB
    return [
        schemas.GrammarMasteryItem(pattern="-(으)ㄹ 줄 알다", mastery_score=0.85, weakness_flags=["misuse of honorifics"]),
        schemas.GrammarMasteryItem(pattern="-고 싶다", mastery_score=0.92, weakness_flags=[])
    ]
...
async def get_vocabulary_mastery(db: Session = Depends(get_db)):
    # This will be implemented later with actual logic to fetch from DB
    return [
        schemas.VocabularyMasteryItem(word_korean="감사합니다", mastery_score=0.95, times_incorrect=1),
        schemas.VocabularyMasteryItem(word_korean="안녕하세요", mastery_score=0.99, times_incorrect=0)
    ]
