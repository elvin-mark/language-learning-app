...
class PracticeAgent:
    ...
    def _get_weakest_grammar_and_vocab(self):
        ...
        """
        weakest_grammar = self.db.query(crud.models.GrammarMastery).order_by(
            crud.models.GrammarMastery.mastery_score.asc()
        ).first()

        drilling_vocab = self.db.query(crud.models.VocabularyMastery).filter(
    ...
    def generate_exercise(self, exercise_request: schemas.ExerciseRequest) -> schemas.ExerciseDetails:
